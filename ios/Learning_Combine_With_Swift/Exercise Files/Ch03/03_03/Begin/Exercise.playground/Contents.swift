...
class MyTests : XCTestCase {
    ...
    func testPublisher() {
        let _ = APIService.getPosts().sink { error in
            ...
            print("Got \(results) posts back")
            XCTAssert(results.count > 0)
            XCTAssert(results.count == 100, "We got \(results.count) insted of 100 posts back")
            XCTAssert(results[0].title == self.expectedTitle, "We got back the title \(results[0].title) insted of \(self.expectedTitle)")
